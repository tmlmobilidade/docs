...
⚠️ Se surgir o erro seguinte, é porque a execução da ferramenta esta a ser bloqueada por restrições à exportação de dados pelo PowerShell. Para contornar esta situação, recomenda-se a utilização da Linha de comandos (CMD).
