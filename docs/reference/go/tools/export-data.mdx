...
*
Se surgir o erro abaixo, é possível que a execução da ferramenta esteja a ser bloqueada. Isso pode ocorrer porque o PowerShell, devido a restrições de permissões, impede a exportação de dados.

Para contornar esta situação, recomenda-se a utilização da Prompt de Comando (CMD).
