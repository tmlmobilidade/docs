...
No `stop_times.txt`, o campo `stop_id` identifica a paragem (plataforma / paragem) atendida neste ponto da viagem. Pelo GTFS, todas as paragens servidas ao longo do trip devem aparecer em linhas de `stop_times`, a paragem pode repetir-se no mesmo trip e várias viagens podem partilhar o mesmo `stop_id`.
