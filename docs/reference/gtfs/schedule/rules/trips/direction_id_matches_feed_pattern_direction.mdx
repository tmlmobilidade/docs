---
title: Sentido da viagem alinhado com a direcção do padrão no feed
description: direction_id_matches_feed_pattern_direction
---
...
No ficheiro `trips.txt`, a regra por linha em `trips.txt` o `direction_id` da viagem tem de coincidir com o segmento de direção codificado no `pattern_id`, no formato esperado pelo perfil do feed (`route_direção_variante` em três partes separadas por `_`). Chave lógica: `direction_id_matches_feed_pattern_direction`.
