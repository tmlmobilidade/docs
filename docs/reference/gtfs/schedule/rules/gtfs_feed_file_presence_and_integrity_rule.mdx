...
- Obrigatórios: um conjunto mínimo fixo (`agency.txt`, `routes.txt`, `trips.txt`, `stop_times.txt`) mais outros ficheiros obrigatórios vindos da regra, sem duplicados; se a tabela não existir, devolve uerro.
- `stops.txt` condicional: se não existir `locations.txt` (tabela `locations`), é obrigatório existir `stops.txt`.
- Calendário: tem de existir pelo menos um de `calendar.txt` ou `calendar_dates.txt`; se faltarem ambos, erro.
...
O método `Validate` devolve se, após estas verificações, aumentou o número total de erros no serviço de mensagens (avisos não alteram esse retorno para “há erro novo” nesta lógica — só `TotalErrors`).

---

### Passa

- Nenhum ficheiro proibido pela regra está presente; os obrigatórios (fixos + regra) estão presentes; `stops` ou `locations` satisfazem a condição; existe calendário ou excepções; `levels` existe quando há elevador em `pathways`; `feed_info` existe quando há `translations`; e não há mistura proibida entre `network_id` nas rotas e tabelas `networks` / `route_networks`.
...
### Avisos (sem ser “falha” de erro)

- Ficheiro na lista de “aviso se em falta” ausente — gera aviso, não erro, conforme `addWarning`.