...
O GTFS recomenda indicar o intervalo em que o conjunto se considera completo e fiável: do primeiro dia (`feed_start_date`) ao último dia (`feed_end_date`). As duas datas podem ficar vazias quando não estão disponíveis. Quando `feed_start_date` tem valor não vazio, esta verificação confirma que corresponde a uma data de serviço válida. A ausência pode ser reportada como obrigatória ou recomendada (excepto quando a verificação não se aplica). Se não for passada severidade à função, usa-se por omissão nível de aviso.

O GTFS recomenda indicar o intervalo em que o conjunto se considera completo e fiável: do primeiro dia (`feed_start_date`) 
