---
title: Horários na paragem válidos de acordo com o enumerado do GTFS
description: has_schedules_valid_enum
---
## Campos que serão validados nesta regra

| Nome do Campo   | Tipo | Presença | Descrição                                                                                                                                                                                                                                                                                                                                         |
| --------------- | ---- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `has_schedules` | Enum | Opcional | Indica se na paragem existem horários divulgados no local.<br/><br/><strong>Opções válidas:</strong><ul><li><code>0</code> — Não aplicável a esta paragem.</li><li><code>1</code> — Paragem sem horários no local.</li><li><code>2</code> — Há horários, mas em mau estado.</li><li><code>3</code> — Há horários e estão em bom estado.</li></ul> |
...
No ficheiro `stops.txt`, o campo `has_schedules` descreve se a paragem dispõe de horários (informação de quadros) e em que situação: não aplicável, sem horários, com horários em mau estado ou com horários em bom estado. O campo é opcional; deixar o campo vazio é aceite no standard - significa não fornecer essa informação. O validador aplica as regras  sobre presença, proibição, valores permitidos (0 a 3) e, se existir, lista de opções permitidas.
...
- Proibido - as regras não permitem o `has_schedules`, mas o valor veio no mesmo fornecido.
