...
O `stop_lat` é a latitude (WGS84) do local: para paragens e plataformas, deve reflectir o postalete ou, na sua falta, o sítio onde o passageiro embarca, e não a via de circulação do veículo. No GTFS a obrigatoriedade depende do `location_type`: obrigatória para paragem/plataforma (0), estação (1) e entrada/saída (2); opcional para nó genérico (3) e área de embarque (4), ainda que a documentação geral do feed recomende coordenadas correctas nesses casos. O validador, para além do standard, aplica regras do AAAA com obrigatoriedade ou recomendação quando a latitude vem vazia em tipos opcionais, e dispensa a verificação se a regra o permitir. Quando o valor existe, tem de ser uma latitude válida (intervalo de graus previsto na validação).
...
- Em falta (ou política de dados) - noutros casos, latitude vazia quando o AAAA exige o preenchimento como erro ou recomendação, sem dispensa.
