...
O `stop_lon` é a longitude (WGS84) do local, em par com a latitude. Para paragens e plataformas, o par de coordenadas deve assentar no postalete ou, em alternativa, no sítio de embarque do passageiro, e não na faixa de rodagem ou carril. A obrigatoriedade no GTFS segue o `location_type`: obrigatória para paragem/plataforma (0), estação (1) e entrada/saída (2); opcional para nó genérico (3) e área de embarque (4). O validador aplica ainda regras do AAAA com obrigatoriedade ou recomendação quando a longitude falta em tipos opcionais, e dispensa a verificação se a regra o permitir. Quando o valor existe, tem de ser uma longitude válida (intervalo de graus previsto na validação).
...
- Em falta (ou política de dados) - noutros casos, longitude vazia quando o AAAA exige o preenchimento como erro ou recomendação, sem dispensa.
