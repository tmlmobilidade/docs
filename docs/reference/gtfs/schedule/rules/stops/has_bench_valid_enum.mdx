---
title: Banco na paragem válido de acordo com o enumerado do GTFS
...
| ------------- | ---- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `has_bench`   | Enum | Opcional | Indica se a paragem tem banco para espera.<br/><br/><strong>Opções válidas:</strong><ul><li><code>0</code> — Não aplicável a esta paragem.</li><li><code>1</code> — Paragem sem banco.</li><li><code>2</code> — Tem banco, mas em mau estado.</li><li><code>3</code> — Tem banco e está em bom estado.</li></ul> |

## O que esta validação faz 

No ficheiro `stops.txt`, o campo `has_bench` descreve se a paragem tem banco, e em que estado: não aplicável, sem banco, com banco em mau estado ou com banco em bom estado. No feed o campo é opcional; deixar o campo vazio é aceite no standard - significa não fornecer essa informação. O validador aplica as regras  sobre presença, proibição, valores permitidos (0 a 3) e, se existir, lista de opções permitidas.
