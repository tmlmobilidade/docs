...
| ID da Regra         | Título                                                        | Descrição                                                                                                                                                                                  | GTFS Standard | GTFS TML     |
| ------------------- | ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------- | ----------- |
| `pattern_id_format` | O `pattern_id` não corresponde a um formato valido            | Valida se o `pattern_id` corresponde a um formato valido, por defeito é XXXX_X_X mas existe algumas exceções que podem conter outros formatos                                              | -             | Obrigatór o  |
| `License_plate`     | A `License_plate` não corresponde formato devidamente correto | Valida se a `License_plate` corresponde ao formato devidamente correto estabelecido que é AA00AA ou 00AA00.<br><br>Pode aparecer aviso se não for introduzido sem "-" por exemplo AA-00-AA | -            Obrigatório rio |
