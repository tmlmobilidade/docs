...
A APEX é o sistema de bilhética utilizado pela Carris Metropolitana, e que está a ser implementado em todos os operadores nacionais. É o que suporta todas as funcionalidades do cartão navegante® — compra e carregamentos de títulos em cartões físicos e digitais, gestão de perfis de cliente, ativação de descontos e muitas outras funcionalidades fundamentais à bilhética dos transportes públicos.

## A bilhética no GO

O GO está intimamente relacionado com a APEX, permitindo uma enorme agilidade no consumo de todas as informações que um sistema de bilhética unificado torna possível. A APEX descreve o catálogo de "produtos" disponíveis num sistema de transportes: a analogia mais simples será a de uma loja online, onde é possível adquirir itens, cada um com uma configuração específica.

Foram criadas várias estruturas de dados simplificadas, que aqui documentamos, para permitir ainda mais agilidade na disponibilização de relatórios e cálculos operacionais. Estas estruturas têm as propriedades essenciais de cada um dos tipos de transação para permitir uma rápida extração de informações sobre a procura do sistema de transportes — passageiros transportados, consolidação de valores, pressão sobre determinados percursos, e muitos outros.

## Tudo são transações

A APEX é uma biblioteca que é instalada nos validadores. É um API local que "descodifica" as mensagens que são trocadas entre os cartões navegante® e o leitor NFC do validador. A APEX implementa o standard internacional [Calypso](https://calypsonet.org/calypso-specifications) que define o tipo de comunicações, assim como as estruturas dos dados que ficam guardados nos cartões e são processados pelos validadores.

Sempre que um cartão navegante® se encosta a um validador, é gerada uma transação. Estas transações não são mais do que pequenos ficheiros de texto que contêm todas as informações necessárias para descrever a ação que acabou de ocorrer. Por exemplo, quando um cartão navegante® toca no validador de um autocarro é gerada uma **transação de validação**, onde ficam registadas informações como o ID da linha (ex: `1715`), o ID do veículo, a data e hora, o número do cartão, o produto utilizado para validar o acesso, e muitos outros dados essenciais para as equipas de gestão.

Aqui está um exemplo de uma transação de validação APEX já descodificada:

```json lineNumbers title="Exemplo de uma transação de validação APEX"
{
	"transactionInfo": {
...
Os GTFS dos operadores são essenciais para configurar corretamente a APEX, pois descrevem que serviços estão disponíveis, que paragem são servidas por cada percurso, e a que zonas tarifárias cada linha pertence.

Nesta secção vamos focar-nos nas principais propriedades dos principais tipos de transação.

[Referência](/reference/files/apex-transactions-reference.pdf)